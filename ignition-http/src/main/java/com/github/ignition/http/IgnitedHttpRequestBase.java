...
import com.github.ignition.http.cache.CachedHttpResponse.ResponseData;
...
public abstract class IgnitedHttpRequestBase implements IgnitedHttpRequest,
    ...
    IgnitedHttpRequestBase(AbstractHttpClient httpClient) {
    ...
    public IgnitedHttpResponse send() throws ConnectException {
        ...
        while (retry) {
            try {
                ...
                if (oldTimeout != IgnitedHttp.getSocketTimeout()) {
                    IgnitedHttp.setSocketTimeout(oldTimeout);
                }
            }
        }
